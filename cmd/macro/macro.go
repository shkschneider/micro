...
func InitFlags() {
	flag.Usage = func() {
		fmt.Println("Usage: macro [OPTIONS] [FILE]...")
		fmt.Println("-clean")
		...
		fmt.Println("-profile")
		fmt.Println("    \tEnable CPU profiling (writes profile info to ./macro.prof")
		fmt.Println("    \tso it can be analyzed later with \"go tool pprof macro.prof\")")
		fmt.Println("-version")
		...
		fmt.Print("\nMacro's options can also be set via command line arguments for quick\nadjustments. For real configuration, please use the settings.json\nfile (see 'help options').\n\n")
		fmt.Println("-option value")
		fmt.Println("    \tSet `option` to `value` for this session")
		fmt.Println("    \tFor example: `macro -syntax off file.c`")
		fmt.Println("\nUse `macro -options` to see the full list of configuration options")
	}
